...
window.onload = function() {
            ...
            const twPromise = () => {
                return new Promise((resolve, reject) => {
                    createjs.Tween.get(card.imgContainer) 
                        ...
                        .to(handPosition,400,createjs.Ease.cubicInOut)
                        .call(()=>{resolve()});
                });
            };
            PromiseArray.push(twPromise());
    ...
    const gameStart = async()=>{
        ...
        if(game.HAND.filter(card=>card.cardType=="Monster").length ==5) {
            game.liblaryOut = true;
            await dealDamage(8000);
            await new Promise<void>(async(resolve, reject) => {
                const winText = genCenterText("YOU WIN");
                winText.shadow = new createjs.Shadow("#000000", 0, 0, 10);
                winText.color = "orangered";
                winText.x = game.centerGrid.x;
                winText.y = game.centerGrid.y;
                winText.font = "100px serif";
                openResultWindow(winText);
                cardContainer.mouseEnabled = false;
                resolve();
            });
        };
    ...
    const gameEnd = async()=>{
        ...
        const DrillFieldImg = new createjs.Bitmap("DrillField.png");
        DrillFieldImg.alpha = 0;
        mainstage.addChild(DrillFieldImg);
        await openResultWindow(winLose);
        // mainstage.setChildIndex(DrillFieldImg,mainstage.numChildren-1);
        await new Promise((resolve, reject) => {
            createjs.Tween.get(DrillFieldImg)
                .to({alpha:1},500)
                .call(()=>{resolve()});
        });
    };
...
墓地のカードをデッキトップに置く`);
    ...
    const updateText = new createjs.Text("", "24px serif","black");
    ...
    const deckRecipe :{json:Object,num:number}[] = [
        {json:status.rightLeg, num:1},
        {json:status.rightHand, num:1},
        {json:status.head, num:1},
        {json:status.leftHand, num:1},
        {json:status.leftLeg, num:1},
        {json:status.fakeTrap, num:35},
    ];
    ...
    const endButton = createButton("諦める", 160, 80, "#0275d8");
    endButton.x = 1300;
    endButton.y = 660;
    endButton.alpha = 0;
    ...
    const resetButton = createButton("再挑戦", 160, 80, "#0275d8");
    resetButton.x = 1300;
    resetButton.y = 750;
    resetButton.alpha = 0;
    ...
    const OpenSelectEffectWindow = (card :Card,messageA:string,messageB:string) => {
        ...
        messageText.innerText = "発動する効果を選択してください"
